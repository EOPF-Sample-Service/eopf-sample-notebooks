...
def makeModel(algorithm,imageCollectionName,variableName, method):
    if method == 'SL2P':
        ...
        networkOptions= dictionariesSL2P.make_net_options_CCRS() #collectionOptions is a dictionary like: S2_SR': {"name": 'S2_SR', "description": 'Sentinel 2A',
    
    colOptions=collectionOptions[imageCollectionName] #LA RIGA INCRIMINATA #colOptions is a dictionary with only the lines pertaining to the imageCollectionName
    netOptions=networkOptions[variableName][imageCollectionName]
    """
...
def prepare_sl2p_inp(
    ...
    if not ref_candidates:
        ...
        if not ref_candidates:
            raise ValueError("No spectral bands found in input to define a reference grid.")
    ...
    for band in netOptions['inputBands']:
        if isinstance(band, str) and band.startswith('B') and (band in ds):
            ...
            if to_10m and (arr.shape != (ref_h, ref_w)):
                ...
                arr = read_sentinel2_safe_image.resample_image(arr, (factor_y, factor_x), interpolation='nearest')
                if arr.shape != (ref_h, ref_w):
                    arr = arr[:ref_h, :ref_w]
                    if arr.shape != (ref_h, ref_w):
                        pad_y = max(0, ref_h - arr.shape[0])
                        pad_x = max(0, ref_w - arr.shape[1])
                        arr = numpy.pad(arr, ((0, pad_y), (0, pad_x)), constant_values=numpy.nan)
    ...
    for band_id, band in enumerate(netOptions['inputBands']):
        ...
        if pb >= 4:
            sl2p_inp[band] = (band_arr + netOptions['inputOffset'][band_id]) * netOptions['inputScaling'][band_id]
        else:
            sl2p_inp[band] = (band_arr + netOptions['inputOffset_before'][band_id]) * netOptions['inputScaling_before'][band_id]
...
def apply_net(array_input, variableName, imageCollectionName, algorithm, method, partition=1):
    
    sl2p_inp = array_input.T 
    sl2p_inp = numpy.reshape(sl2p_inp, (sl2p_inp.shape[0], sl2p_inp.shape[1], -1)) # reshaping the input array sl2p_inp so that it has: 1st dim the same as shape[0], 
    #2nd dim same as shape[1], 3rd dim inferred
    
    networkOptions= dictionariesSL2P.make_net_options_CCRS()
    collectionOptions = (dictionariesSL2P.make_collection_options_CCRS(algorithm))

    netOptions=networkOptions[variableName][imageCollectionName] # These are the network specific option (scaling, offset, column and band names, etc.)
    colOptions=collectionOptions[imageCollectionName] # These are the collection specific options, specifically calling the different NN weights and biases)
    
    # prepare SL2P networks
    SL2P,errorsSL2P=makeModel_CCRS(algorithm,netOptions,colOptions) # specific function for CCRS
    
    # Run SL2P (actual NN application part)
    print('Run SL2P...\nSL2P start: %s' % datetime.now())
    estimate, networkID = toolsNets.wrapperNNets_CCRS(SL2P, netOptions, colOptions, sl2p_inp, partition=partition) 
    error, uncertainty = toolsNets.wrapperNNets_CCRS(errorsSL2P, netOptions, colOptions, sl2p_inp, partition=partition)
    print('SL2P end: %s' % datetime.now())
    ...
    out_array = numpy.stack([estimate, networkID, error, qc_input.flatten(), qc_output], axis=1)
...
def SL2PCCRS(samples_array, variableName, imageCollectionName, tile, method):
    ...
    partition_array = make_partition(tile, (samples_array.shape[1], samples_array.shape[2]))
    ...
    for partition_value in numpy.unique(partition_flat):
        ...
        mask = partition_flat == partition_value # Creates a mask to filter the samples_array based on the current partition value
        selected_inputs = samples_flat[mask] # Applies the mask to the samples_array to get the rows corresponding to the current partition

        # Apply the SL2P algorithm to the filtered DataFrame for the current partition
        out_values = apply_net(selected_inputs, variableName, imageCollectionName, algorithm, method, partition=partition_value)
